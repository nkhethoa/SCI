...
class Learners extends CI_Controller{
    ...
    //check user credentials
       $is_logged_in = ($this->session->userdata('is_logged_in')) ? $this->session->userdata('is_logged_in') : FALSE;
        ...
        //disallow if the user is the guardian wanting to access the controller
        if (identify_user_role($_SESSION['userID'])==='guardian') {
    ...
    public function sendLearnerMessage()
    {
    ...
    if ($this->form_validation->run()===FALSE) {
        ...
        if($this->input->post('mess_ID')==''){
          //assign message variables into db friendly variables
            $msg = array(
                'title'=>html_escape($this->input->post('lsubj')),
                'body'=>html_escape($this->input->post('lmsg')),
                'to_user_id'=>html_escape($this->input->post('userlearnerID'))
              );
